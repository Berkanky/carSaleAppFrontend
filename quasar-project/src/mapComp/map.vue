<template>
  <q-dialog
    v-model="this.store.mapCompActive"
  ...
  methods:{
    async getSelectedLocationInformations(newLocation){
      ...
      if(data.results.length){
        this.store.locationFilterData.city = data.results[0].components.province ?? ''
        this.store.locationFilterData.country = data.results[0].components.country ?? ''
        this.store.locationFilterData.fullAddress =  data.results[0].formatted ?? ''
        this.store.locationFilterData.lat = newLocation.lat ?? ''
        this.store.locationFilterData.lng = newLocation.lng ?? ''
        console.log('locationFilterData',this.store.locationFilterData)
      }
    },
    onChangeLocation(e){
